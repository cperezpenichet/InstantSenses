...
define('lib/jquery.lang.string',['jquery'],function( $ ) {
...
define('lib/jquery.class',['jquery','lib/jquery.lang.string'], function( $ ) {
...
define('lib/jquery.model',['jquery','lib/jquery.class'], function( $ ){
...
define('models/publisher',['jquery', 'lib/jquery.model', 'require',
			], function($, Model, require) {
				
				
		Model('InstantPlaces.Model.Publisher', {
			findAll : 'GET http://testapi.instantplaces.org/publish/publishers',
			models : function(data) {
				return this._super(data.items);
			},
			findOne : 'GET http://testapi.instantplaces.org/publish/publishers/{id}'
		}, {
			getSignsCreated : function(params, success, error) {
				var url;
				// get items link
				$.each(this.links, function(i, link) {
					if(link.rel === 'signs-created') {
						url = link.href;
						//break for each
						return false;
					}
				});
				//make request and convert to models
				return $.ajax({
					url : url,
					type : 'get',
					dataType : 'json sign.models',
					data : params,
					success : success,
					error : error
				})
		...
		return InstantPlaces.Model.Publisher;
	
});

define('models/item',['jquery', 'lib/jquery.model',
			'models/publisher'], function($, Model) {
	Model('InstantPlaces.Model.Item', {
		attributes : {
			publisher : 'InstantPlaces.Model.Publisher.model',
		},
		findAll : 'GET http://testapi.instantplaces.org/publish/items',
		models : function(data) {
			if('items' in data) {
				return this._super(data.items);
			}
		...
		findOne : 'GET http://testapi.instantplaces.org/publish/items/{id}'
	}, {});
});

define('models/collection',['jquery', 'lib/jquery.model',
			'models/publisher', 'models/item'], function($, Model) {
	Model('InstantPlaces.Model.Collection', {
		attributes : {
			publisher : 'InstantPlaces.Model.Publisher.model',
		},
		findAll : 'GET http://testapi.instantplaces.org/publish/collections',
		findOne : 'GET http://testapi.instantplaces.org/publish/collections/{id}',
		models : function(data) {
			return this._super(data.items);
		}
	}, {
		getItems : function(params, success, error) {
			var url;
			// get items link
			$.each(this.links, function(i, link) {
				if(link.rel === 'expressionCollectionItems') {
					url = link.href;
					//break for each
					return false;
				}
...
define('models/sign',['jquery', 'lib/jquery.model',
			'models/publisher'], function($, Model) {

	Model('InstantPlaces.Model.Sign', {
		attributes : {
			publisher : 'InstantPlaces.Model.Publisher.model',
		},
		findAll : 'GET http://testapi.instantplaces.org/publish/signs',
		models : function(data) {
			return this._super(data.items);
		},
		findOne : 'GET http://testapi.instantplaces.org/publish/signs/{id}'
	}, {});

});

define('models/situatedIdentity',['jquery', 'lib/jquery.model', 'models/item'], function($, Model) {

	Model('InstantPlaces.Model.SituatedIdentity', {
		attributes : {
			items : 'InstantPlaces.Model.Item.models'
		},
		models : function(data) {
			return this._super(data.items);
		},
		findOne : 'GET http://testapi.instantplaces.org/places/situation/situatedIdentities/{id}'
	}, {});

});

define('models/place',['jquery', 'lib/jquery.model',
			'models/sign', 'models/situatedIdentity'], function($, Model) {

	Model('InstantPlaces.Model.Place', {
		findAll : 'GET http://testapi.instantplaces.org/places',
		models : function(data) {
			return this._super(data.items);
		},
		findOne : 'GET http://testapi.instantplaces.org/places/{id}'
	}, {
		...
		getSigns : function(params, success, error) {
			var url = this.getLink('signs');
			if(url) {
				//make request and convert to models
				return $.ajax({
					url : url,
		...
		getSituatedIdentities : function(params, success, error) {
			var url = this.getLink('situatedIdentities');
			if(url) {
				//make request and convert to models
				return $.ajax({
					url : url,
					type : 'get',
					dataType : 'json situated_identity.models',
					data : params,
					success : success,
					error : error
				});
	...
	return InstantPlaces.Model.Place;
});

require.config({
	paths : {
		jquery : 'lib/jquery-1.7.1'//,
		//jquerymx : 'lib/jquerymx-3.2.custom'
	}
});

require(['models/collection','models/item','models/place','models/publisher','models/sign','models/situatedIdentity'], function() {

	/*InstantPlaces.Model.Place.findAll({},function(data){
		console.log(data);
	});
	
	InstantPlaces.Model.Publisher.findAll({},function(data){
		console.log(data);
